...
class SearchResult:
    ...
    wildcards: Dict[str, Union[lal.AdaNode, None, list]]
    ...
    def _are_identical(self, root1: lal.AdaNode, root2: lal.AdaNode) -> bool:
        ...
        if (
            ...
            return False  # the leaf is a regular text node it doesn't pass the comparison
        ...
        plural_wildcards_no = sum(_is_plural_wildcard(child.text) for child in filter_children2)
    ...
    def wildcard_list_matching(self, root1: lal.AdaNode, root2: lal.AdaNode, plural_wildcards_no: int):
        ...
        # we preliminarily check the lower bound of the number of children in root1
            ...
            while i < len(root1.children) and j < len(root2.children):
                if j in wildcard_indexes:  # if the current element of root2 is a wildcard
                    ...
                    if root2.children and not self._are_identical(root1.children[i], root2.children[j]):
                        multi_wildcard_value.append(root1.children[i])
                    ...
                    # indeed matches the last wildcard that appeared on root2
                        ...
                        if wildcard_indexes and [index for index in wildcard_indexes if index < j]:
                            last_wildcard = max([index for index in wildcard_indexes if index < j])
                            ...
                            ignore_nonexistant = True if last_wildcard == wildcard_indexes[-1] else False
    ...
    def _wild_comparison(self, root1: Union[lal.AdaNode, None], root2: lal.AdaNode) -> bool:
    ...
    def _wild_comparison_multi(self, multi: list, root2: lal.AdaNode, ignore_nonexistant=False) -> bool:
        for key in self.wildcards.keys():
            if _ignore_semicolon_comparison(key, root2.text):
